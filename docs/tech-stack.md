...
| バックエンド | Supabase (`@supabase/supabase-js` v2) | 確定済み。Postgres・Realtime・**匿名 Auth**・Edge Functions を使用（認証方式の節を参照） |
...
| ログイン（匿名 → 正式アカウント昇格） | 端末をまたぐ利用・外部カレンダー同期が必要になった場合 | Supabase は匿名 → 正式アカウントへの昇格を公式サポートしており、`auth.uid()` を維持したままログインを追加できる。テーブル・RLS・コードの構造変更は不要 |
| 外部カレンダー双方向同期 | ログイン導入とセットで（OAuth が前提のため） | 今回は ics エクスポートのみ。ただし**後入れ可能な構造は最初から確保する**: ① 予定テーブルに出自フィールド（プラン / 個人 / 外部）を持たせる ② 外部 API 呼び出しは Edge Function 経由に隔離。追加パッケージ不要の見込み（REST 直接呼び出し。googleapis SDK は Deno で重いため使わない） |
...
| 不採用 | 理由 |
| --- | --- |
| Google ログイン（OAuth 全般） | ログイン画面・OAuth 設定に時間を使わず、コアに集中する（チーム方針）。匿名 Auth のみで開始し、必要になれば匿名昇格で後から足せる（条件付き項目参照） |
...
**匿名 Auth のみを採用し、Google ログインは入れない**（Issue #16）。QR / URL からログイン画面なしで即参加でき、ニックネームのみ入力する。

受け入れる制約:

- 身元がブラウザ（localStorage）単位になるため、**端末をまたぐ連続性はない**（画面 3 の「個人予定」は当面端末単位）
- localStorage を消すと旅行メンバーシップとの紐付きが失われる（旅行データ自体は残る）

これを受け入れられる根拠: Supabase の**匿名 → 正式アカウント昇格**により、`auth.uid()` を維持したままログインを後から追加できる。「必要になったら足す」が構造変更なしで成立する（条件付き項目参照）。
